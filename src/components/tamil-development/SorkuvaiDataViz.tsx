...
function FloatingWord({ word, onComplete }: { word: ActiveWord, onComplete: () => void }) {
  ...
  return (
    ...
    >
      <div className="font-display text-[#fcd567] drop-shadow-[0_0_8px_rgba(252,213,103,0.4)] text-sm md:text-lg whitespace-nowrap">
        {word.ta}
      </div>
      {word.en && (
        <div className="font-mono text-[0.55rem] md:text-[0.6rem] uppercase tracking-wider text-[#fcd567]/80 mt-0.5 whitespace-nowrap">
          {word.en}
        </div>
      )}
    </div>
        ...
        <div className="relative w-full max-w-6xl h-[400px] md:h-[500px] flex items-center justify-center mb-16">
          ...
          <div ref={heroRef} className="relative z-[30] text-center bg-gradient-to-b from-[var(--color-tamil-ink)]/90 to-[var(--color-tamil-ink)]/70 backdrop-blur-xl p-8 md:p-16 rounded-[3rem] border border-[var(--color-tamil-gold)]/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
            <div className="text-[#fcd567] drop-shadow-[0_0_20px_rgba(252,213,103,0.15)]">
              <span ref={countRef} className="font-display text-5xl md:text-8xl font-bold tracking-tight leading-none">{DATA.wordsUploaded.formatted}</span>
            </div>
            <div className="mt-6 flex flex-col items-center gap-2">
              <span className="font-sans text-xl md:text-3xl text-white font-medium">
                {locale === 'ta' ? 'பதியப்பட்ட தமிழ்ச் சொற்கள்' : 'Tamil words uploaded'}
              </span>
              <span className="text-xs uppercase tracking-widest text-[var(--color-tamil-gold)]/70 border-t border-[var(--color-tamil-gold)]/20 pt-2 mt-1 font-mono">
...
function Milestone({ value, title, subtitle, context }: { value: string, title: string, subtitle: string, context: string }) {
  return (
    <div className="milestone flex flex-col items-center text-center p-8 border-t border-[var(--color-tamil-gold)]/20 bg-[var(--color-tamil-ink)]/50 backdrop-blur-md rounded-b-2xl relative group shadow-xl">
      ...
      <div className="font-sans text-lg text-[#fcd567] font-medium mb-4">{title}</div>
      <div className="text-[0.65rem] font-bold tracking-widest uppercase text-white/50 mb-2 font-mono">{subtitle}</div>
      <div className="text-[0.65rem] uppercase tracking-wider text-[var(--color-tamil-red)] font-mono">{context}</div>
    </div>
  );
}
